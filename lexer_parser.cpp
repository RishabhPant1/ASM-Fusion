...
#include <sstream>
#include <set>
#include "asm_fusion.h" // Include the shared header
#include "converter.h"  // Include the converter header
using namespace std;
...
private:
    ...
    Token readWord() {
        ...
        // Check if next character is colon (for labels)
        skipWhitespace();
...
public:
    ...
    optional<ParsedInstruction> parseLine() {
        ...
        // Handle label-only lines or labels with instructions
        if (pos < (int) tokens.size() && tokens[pos].type == TokenType::Label) {
            ...
            // If this is a label-only line, return just the label
        ...
        return instr; // Return instruction even if it only has a label
    }
...
public:
    ...
    void generateCCode(const string& outputFilename) {
        ...
        // Generate function declarations for called labels
        ...
        // Generate function definitions for labels that are called
        for (const string& target : jumpTargets) {
            if (labels.count(target)) {
                outFile << "\nvoid label_" << target << "() {\n";
